...
- Vision (VL): when validating DEMO frames or milestone screenshots, spawn a vision-capable subagent when available. Confirm the voxel world looks coherent (terrain, HUD, not black canvas) — never invent block rules. Without VL: Read demo/frames/ yourself; log PASS/FAIL in DEMO.md.
...
0. PRE-PR VISUAL (mandatory — BEFORE commit / push / open or update PR):
   ...
   - If a smart / vision-capable model is available: spawn it to analyze that
     screenshot. Require PASS (coherent for this milestone — expected UI/world
     visible, not black canvas, not a blank error page). On FAIL: fix, re-shot,
     re-validate — do **not** push or open/update the PR.
   - If no smart/VL model: Read the screenshot yourself; log PASS/FAIL + path in
     PROGRESS.md. FAIL still blocks push/PR.
   - Record in PROGRESS.md NOW: screenshot path + validator (smart/VL id or
     "self-read") + PASS. No PASS → no push.
...
2. VL / smart (required when available): spawn smart/vision model to analyze
   that screenshot. Require PASS ("playable for this phase"). On FAIL: fix,
   re-shot, re-validate — do not advance phase, do not push.
   Without VL: Read the screenshot yourself; FAIL still blocks.
