...
4. `shared/design.md` (+ ARCHITECTURE.md if present), CONTENT/FEATURES/SOAK/DEMO
   as relevant.
5. Role pack if this run uses roles: `example-prompts/roles/{feature,fix}.md`
   (+ domain appendix). If a genre pack was the initial law
   (`example-prompts/games/<name>.md`), re-open **that file** for ACCEPT only —
   do not restart M1 if later work exists.
6. `AGENTS.md` / `BOILERPLATE.md` / `MODEL_STACKS.md` — branch + gate + stack;
   do not rewrite BOILERPLATE_OWNED.
...
0. **FIX-FIRST:** If BUGS ## Open has blocker/playability → behave as
   `roles/fix.md` (FIX-ONLY). No new features.
