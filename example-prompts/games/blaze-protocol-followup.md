...
WHAT THIS JOB WAS
- Full overnight law: `blaze-protocol.md` (same `example-prompts/games/` folder, or
...
- Game: Blaze Protocol — Phaser 3 twin-stick arena shooter
- Branch pattern: `agent/blaze-protocol-*` (use the existing run branch in PROGRESS /
  git; do not invent a second product tree)
- Stack: Phaser 3 + TypeScript + Vite (2D sprites; solo)
...
1. `git status` / current branch / recent commits. Confirm this is the lab clone
   on `agent/blaze-protocol-…` (or the run id recorded in PROGRESS).
...
THEN CONTINUE
1. create_goal with the same overnight objective as `blaze-protocol.md` and
   max_goal_rounds ≥ 200. Never mark complete on your own (human kills) — Phase 4 runs forever
   unless `blaze-protocol.md` explicitly allows STOP_AFTER_DEMO.
...
3. Obey `blaze-protocol.md` LIVE LOOP before every push/PR — including **PRE-PR VISUAL**
...
If `blaze-protocol.md` is missing from disk, recover rules from PROGRESS + CODE + this
