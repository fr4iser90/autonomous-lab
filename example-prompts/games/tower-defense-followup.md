...
WHAT THIS JOB WAS
- Full overnight law: `tower-defense.md` (same `example-prompts/games/` folder, or
...
- Game: Online 2D tower defense — multiplayer build / waves
- Branch pattern: `agent/tower-defense-*` (use the existing run branch in PROGRESS /
  git; do not invent a second product tree)
- Stack: Canvas2D + WebSocket server + Vite client (no Three/WebGL required)
...
1. `git status` / current branch / recent commits. Confirm this is the lab clone
   on `agent/tower-defense-…` (or the run id recorded in PROGRESS).
...
THEN CONTINUE
1. create_goal with the same overnight objective as `tower-defense.md` and
   max_goal_rounds ≥ 200. Never mark complete on your own (human kills) — Phase 4 runs forever
   unless `tower-defense.md` explicitly allows STOP_AFTER_DEMO.
...
3. Obey `tower-defense.md` LIVE LOOP before every push/PR — including **PRE-PR VISUAL**
...
If `tower-defense.md` is missing from disk, recover rules from PROGRESS + CODE + this
