...
WHAT THIS JOB WAS
- Full overnight law: `moba.md` (same `example-prompts/games/` folder, or
...
- Game: Online 3D MOBA-lite — arena, heroes, minions
- Branch pattern: `agent/moba-*` (use the existing run branch in PROGRESS /
...
- If PROGRESS NOW has STOP_AFTER_DEMO: true and Phase 3 is truly done → mark goal
  complete and STOP (do not enter Phase 4).
...
THEN CONTINUE
1. create_goal with the same overnight objective as `moba.md` and
   max_goal_rounds ≥ 200. Never mark complete on your own (human kills) — Phase 4 runs forever
   unless `moba.md` explicitly allows STOP_AFTER_DEMO.
...
3. Obey `moba.md` LIVE LOOP before every push/PR — including **PRE-PR VISUAL**
...
If `moba.md` is missing from disk, recover rules from PROGRESS + CODE + this
