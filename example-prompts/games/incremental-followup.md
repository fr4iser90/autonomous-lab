...
WHAT THIS JOB WAS
- Full overnight law: `incremental.md` (same `example-prompts/games/` folder, or
...
- Game: Incremental / idle DOM game (economy + prestige)
- Branch pattern: `agent/incremental-*` (use the existing run branch in PROGRESS /
...
THEN CONTINUE
1. create_goal with the same overnight objective as `incremental.md` and
   max_goal_rounds ≥ 300. Never mark complete on your own (human kills) — Phase 4 runs forever
   unless `incremental.md` explicitly allows STOP_AFTER_DEMO.
...
3. Obey `incremental.md` LIVE LOOP before every push/PR — including **PRE-PR VISUAL**
...
If `incremental.md` is missing from disk, recover rules from PROGRESS + CODE + this
