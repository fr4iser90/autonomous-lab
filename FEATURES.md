...
## Shipped
- M12: **Save v6** — `settings` field (`autoAscend` boolean) in payload; v5→v6 migration loads defaults. **Offline check-back** (`src/economy/save.ts` `computeCheckBack`): calculates 25 % of up to 8 hours (28 800 s) of offline production at the saved `perSec` rate; capped, zero if production=0, not-applied when gain < 0.5; delta applied to signal on load. **ClearSave** (`clearSave()`) removes localStorage key; UI button in settings row with page reload. **Mobile layout** — responsive CSS breakpoints at 480px (single-column grid, flex-wrap relay rows, full-width buy buttons, centered shop tabs) and 360px (smaller fonts, larger click button). **Balance retune**: threshold growth lowered 3× → 1.5× so simulateToLayer(20) reaches within 2M ticks.
